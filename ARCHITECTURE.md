...
- scheme.dat (схема полей для документов)
    - uint64_t - число полей
    - для каждого поля: uint8_t тип (0=int, 1=string), uint64_t длина имени, байты имени

- docs.dat (документы по схеме)
    - uint64_t - число документов
    - для каждого документа, для каждого поля схемы по порядку: если int - int64_t, если string - uint64_t длина, затем байты

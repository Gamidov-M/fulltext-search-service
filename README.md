...
- [x] Стоп-слова при индексации и в запросе
...
Инкрементально (без полной пересборки индекса): query-параметр `incremental=true` (или `1`, `yes`).

Ключ документа - первое в схеме коллекции поле типа `int` (обычно `id`), 
при совпадении ключа текст и индекс для этой строки обновляются на месте, новые ключи дописываются. 

Удаление отдельных документов через полную перезагрузку набора без `incremental`.

```bash
curl -X POST 'http://127.0.0.1:8000/indexes/products/documents?incremental=true' \
  -H 'Content-Type: application/json' \
  -d '[{"content": {"id": 3, "name": "Новый документ"}}]'
```
...
Пример ответа (`incremental=true`)
